...
public class Data {
	...
	public double[][] normalizeExternalDataset(double[][] dataset) {
		int externalRows = dataset.length;
		double[][] normed = new double[externalRows][];
		for (int i=0; i<externalRows; i++) {
			normed[i] = new double[cols+1];
			normed[i][0] = dataset[i][0];
			for (int j=0; j<cols; j++) {
				normed[i][j+1] = reweightPoint(i, j, dataset[i][j+1]);
			}
		}
		return normed;
	}
